...
export default function CourseManagement({ noLayout = false, hideStats = false }) {
  ...
  const courseColumns = useMemo(() => [
    ...
    },
    {
      ...
      header: "INSTITUTE",
      cell: ({ row }) => (
        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${row.getValue("institute") === 'GIT' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'
          }`}>
